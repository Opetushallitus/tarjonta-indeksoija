...
(deftest hierarkia-test
  ...
  (testing "Fetching hierarkia via cache should"
    ...
    (testing "return correct koulutustoimijat"
      (let [res (cache/get-hierarkia-cached)
        ...
        (is (= (get @res "1.2.246.562.10.594252633210") {:oid "1.2.246.562.10.594252633210" :kotipaikkaUri "kunta_398" :nimi {:fi "Koulutuskeskus Salpaus -kuntayhtymä" :sv "Koulutuskeskus Salpaus -kuntayhtymä sv"} :status "AKTIIVINEN"
                                                         :organisaatiotyypit ["organisaatiotyyppi_01"] :childOids ["1.2.246.562.10.66634895871", "1.2.246.562.10.99999999999", "1.2.246.562.10.81934895871"]}))
        (is (= (get @res "1.2.246.562.10.53814745062") {:oid "1.2.246.562.10.53814745062" :kotipaikkaUri "kunta_091" :nimi {:en "Helsingin Yliopisto" :fi "Helsingin Yliopisto" :sv "Helsingin Yliopisto"} :status "AKTIIVINEN"
                                                        :organisaatiotyypit ["organisaatiotyyppi_01"] :childOids ["1.2.246.562.10.112212847610", "1.2.246.562.10.81927839589", "1.2.246.562.10.39218317368"]}))
        (is (= (get @res "1.2.246.562.10.73481747332") {:oid "1.2.246.562.10.73481747332" :kotipaikkaUri "kunta_759" :nimi {:en "Soinin kunta" :fi "Soinin kunta" :sv "Soinin kunta"} :status "AKTIIVINEN"
                                                        :organisaatiotyypit ["organisaatiotyyppi_01", "organisaatiotyyppi_07", "organisaatiotyyppi_09"] :childOids ["1.2.246.562.10.53670619591"]}))
    ...
    (testing "return correct toimipisteet, grandchildren toimipisteet relocated under oppilaitos"
      (let [res (cache/get-hierarkia-cached)
        ...
        (is (tools/contains-same-elements-in-any-order? ["1.2.246.562.10.777777777993", "1.2.246.562.10.777777777991", "1.2.246.562.10.61397511793", "1.2.246.562.10.45501388404",
                                                         ...
                                                         "1.2.246.562.10.001010101012", "1.2.246.562.10.001010101021", "1.2.246.562.10.001010101022", "1.2.246.562.10.001010101023"] toimipisteet))))
    (testing "Toimipiste -type of grandchildren relocated under oppilaitos"
      (let [res (cache/get-hierarkia-cached)]
        ...
        (is (= (:parentToimipisteOid (get @res "1.2.246.562.10.73683668492")) "1.2.246.562.10.73307006806"))
        (is (= (:parentOid (get @res "1.2.246.562.10.27277224708")) "1.2.246.562.10.39218317368"))
        (is (= (:parentToimipisteOid (get @res "1.2.246.562.10.27277224708")) "1.2.246.562.10.94997228401"))
        (is (= (:parentOid (get @res "1.2.246.562.10.24134663046")) "1.2.246.562.10.39218317368"))
        (is (= (:parentToimipisteOid (get @res "1.2.246.562.10.24134663046")) "1.2.246.562.10.37753840224"))
        (is (= (:parentOid (get @res "1.2.246.562.10.38836199083")) "1.2.246.562.10.39218317368"))
        (is (= (:parentToimipisteOid (get @res "1.2.246.562.10.38836199083")) "1.2.246.562.10.94639300915")))))
  ...
  (testing "return OPH data correctly"
    (is (= (cache/get-hierarkia-item "1.2.246.562.10.00000000001") {:oid "1.2.246.562.10.00000000001"
                                                                    :status "AKTIIVINEN"
