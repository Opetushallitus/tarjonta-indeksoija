 ...
 (deftest s3-client-test
   (testing "s3 client should"
     (testing "store pictures and refresh pictures"
       (is (= 0 (count (s3/list-keys "organisaatio" oid))))
       (let [pics [{:kieliUri "kieli_fi", :filename "hau.txt", :mimeType "text/plain", :base64data ""}]]
         (client/refresh-s3 oid "organisaatio" pics))
       (is (= 0 (count (s3/list-keys "organisaatio" oid))))
       (let [pics [{:kieliUri "kieli_fi", :filename "vuh.txt", :mimeType "text/plain", :base64data (b64/encode "vuh")},
         ...
         (client/refresh-s3 oid "organisaatio" pics))
       (is (= 3 (count (s3/list-keys "organisaatio" oid))))
       (let [pics [{:kieliUri "kieli_fi", :filename "hau.txt", :mimeType "text/plain", :base64data (b64/encode "hau")}]]
         (client/refresh-s3 oid "organisaatio" pics))
       (is (= 1 (count (s3/list-keys "organisaatio" oid))))))))